<?php
  ...
  <form   class="accountSetting-form" method="POST" action="./accountSetting.php" enctype="multipart/form-data">
...
<?php 
  if($_SERVER['REQUEST_METHOD']=='POST'){
    ...
    //if badge1 is waiting/verified(User already submitted document)
    if($user['badge1'] != "unsubmitted"){
      //If user upload other document, should delete previous one and update.
      if(uploadfile('./img/ref_img/','refImg')=='true'){
        unlink("../img/ref_img/".$user['refImg'].""); // delete privious file
      }
      $refImg = $_FILES['refImg']['name'];
      $badge1 = $user['badge1'];
    }else{
      //if badge1 is unsubmitted
      if(uploadfile('./img/ref_img/','refImg')=='true'){
        $refImg = $_FILES['refImg']['name'];
        $badge1 ='waiting';
      }else{
    ...
    if($user['badge2'] != "unsubmitted"){
      //If user upload other document, should delete previous one and update.
      if(uploadfile('./img/tam_img/','tamImg')=='true'){
        unlink("../img/tam_img/".$user['tamImg'].""); // delete privious file
      }
