    ...
    foreach($userArray as $users){
      ...
      foreach($users as $field=>$userDetail){
        if($users['user_id'] == $id){
          switch($field){
            case 'badge1':
              echo '<td><form method="POST" action="./adminEditUser.php""><select class="badge-select" name="badge1"><option>unsubmitted</option><option>waiting</option><option>verified</option></select></td>';
            break;
            case 'badge2' :
              echo '<td><form method="POST" action="./adminEditUser.php""><select class="badge-select" name="badge2"><option>unsubmitted</option><option>waiting</option><option>verified</option></select></td>';
            break;
            default :
